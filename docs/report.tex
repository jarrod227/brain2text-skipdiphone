...
despite D having better PER.  The skip-diphone auxiliary loss encourages
the encoder to encode longer-range context, which improves phone
discrimination but disrupts the blank/phone temporal alignment that the
WFST decoder relies on.

\textbf{2. Smoothness helps WER.}  Variant C ($\lambda=0.005$) attains
the best 3-gram WER overall (19.01\%), beating B by 0.22 pp.  Temporally
smoother phone posteriors appear to produce cleaner CTC lattices.
...
(18.07\%), and smoothness-only (C) wins 3-gram WER (19.01\%).  The
responsible mechanism is a calibration mismatch introduced by diphone
marginalization, compounded by the skip-diphone head's disruption of
blank/phone temporal alignment.
